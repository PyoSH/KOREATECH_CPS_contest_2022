...
void donode(int ndvl, int cndvl = 'i') {
    if (EEPROM.read(nodeindex) == 105 || EEPROM.read(nodeindex) == 255 || cndvl != 'i') EEPROM.write(nodeindex, cndvl);
...
void loop() {
    ...
    if (button == 1 && pre_button == 0 || start == 1)
    {
        ...
        if (STEP != pSTEP)
        {
            ...
            if (!(!(rs >= 170) && !(ls >= 170) && (fs >= 90))) {
                if (EEPROM.read(readindex) == 108) ls = 100; // ls_on false
                else if (EEPROM.read(readindex) == 114) rs = 100; // rs_on false
                else if (EEPROM.read(readindex) == 102) fs = 50; // fs_on false
                readindex++;
            }
