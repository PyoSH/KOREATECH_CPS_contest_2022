...
double kp1 = 2, kp2 = 4; //비례제어 (튜닝필요) (0) 1, 2 (1) 
const int b = 230; //벽 사이간 거리 (튜닝필요) (0) 250
const int a = 60;  //센서 간격 거리 (튜닝필요) (0)79 
...
bool U_turn(double ct)
{
	...
	case 0:
		...
		rw = -180;
		lw = 180;
		if ((ct - chkTime) >= 1.47) // 0.6은 회전시간 (시간 튜닝 필) 
			// (0) 0.6 (1) 0.8 (2) 1.5-230 (3) 1.0 (4) 1.3 (5) 1.47
		{
			m_step++;
			chkTime = ct;
			delayChkTime = ct;
...
bool RL_Turn(double ct, int turn)
{
	...
	case 0:
		...
		{
			rw = lw = 250; //튜닝 필
			if (((ct - chkTime) >= 1.0) || (fs <= 30)) //튜닝 필 (0) 0.2 (1) 0.3 (2) 0.6 (3) 0.9 (4) 1.4
			{
		...
		else if (turn == -1) //좌회전시 
		{
			...
			if ((ct - chkTime) >= 0.7 || (fs <= 30)) //튜닝 필 (0) 0.1 (1) 0.3 (2) 0.6 (3) 0.7
			{
				/*if (fs <= 30) {
					analogWrite(7, 100);
				}
